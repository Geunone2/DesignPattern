...
class ShortWay {
    ...
    calculateWay(start, destination) {
        ...
        return {baseResult, route: `${baseResult.route} (최단 경로 추천)`, distance: baseResult.distance - 50};
    }
}
...
class GoodViewWay {
    ...
    calculateWay(start, destination) {
        const baseResult = this.navigation.calculateWay(start, destination);
        return {baseResult, route: `${baseResult.route} (경치 좋은 경로 추천)`, distance: baseResult.distance + 50};
    }
}

const geunThrWay = new GeunThrWay();

console.log('GeunThr-내비게이션 추천 경로');
console.log('=================================================');
const shortestRouteNavigation = new ShortWay(geunThrWay);
...
const scenicRouteNavigation = new GoodViewWay(geunThrWay);
